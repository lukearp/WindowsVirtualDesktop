/root/.dotnet/tools/certificate-tool add --base64 $Env:base64 --password $Env:certPass --thumbprint $Env:thumbprint --store-name My
$HostpoolName = $Env:HostpoolName
$HostPoolResourceGroupName = $Env:HostPoolResourceGroupName
$BeginPeakDateTime = $Env:BeginPeakDateTime
$EndPeakDateTime = $Env:EndPeakDateTime
$TimeZoneId = $Env:TimeZoneId
$FullPeakHours = $Env:FullPeakHours
$PeakTimeUserBuffer = $Env:PeakTimeUserBuffer
$SessionThresholdPerCPU = $Env:SessionThresholdPerCPU #int
$PeakMinimumNoOfRDSH = $Env:PeakMinimumNoOfRDSH
$MinimumNumberOfRDSH = $Env:MinimumNumberOfRDSH #int
$LimitSecondsToForceLogOffUser = $Env:LimitSecondsToForceLogOffUser #int
$LogOffMessageTitle = $Env:LogOffMessageTitle #"Auto Log Off"
$LogOffMessageBody = $Env:LogOffMessageBody #"Your session host is powering down. Please save your work.  Once logged off you can reconnect to an available host."
$MaintenanceTagName = $Env:MaintenanceTagName

$WVDConnection = New-Object -TypeName psobject -Property @{
    ApplicationId = $Env:WVDApplicationId
    TenantId = $Env:WVDTenantId
    CertificateThumbprint = $Env:thumbprint
    SubscriptionId = $Env:WVDSubscriptionId
}

$Connection = New-Object -TypeName psobject -Property @{
    ApplicationId = $Env:AzApplicationId
    TenantId = $Env:AzTenantId
    CertificateThumbprint = $Env:thumbprint
    SubscriptionId = $Env:AzSubscriptionId
} 
...
$functions = {
    ...
    if ($isPeak) {
        ...
        $SkipSessionhosts = @()
        $login = Login-ToAzureForWorkspace -WVDConnection $WVDConnection
        $HostPoolUserSessions = Get-AzWvdUserSession -HostPoolName $HostpoolName -ResourceGroupName $HostPoolResourceGroupName -SubscriptionId $WVDConnection.SubscriptionId | ?{$_.SessionState -eq "Active"}
        $login = Login-ToAzureForSessionHosts -Connection $Connection

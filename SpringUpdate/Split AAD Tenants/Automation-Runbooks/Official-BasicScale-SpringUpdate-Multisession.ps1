...
try {
	...
	if (!$SkipAuth) {
		...
		try {
			$AzAuth = Connect-AzAccount -ApplicationId $Connection.ApplicationId -CertificateThumbprint $Connection.CertificateThumbprint -TenantId $Connection.TenantId -SubscriptionId $Connection.SubscriptionId -ServicePrincipal
			$WvdAuth = Connect-AzAccount -ApplicationId $WVDConnection.ApplicationId -CertificateThumbprint $WVDConnection.CertificateThumbprint -TenantId $WVDConnection.TenantId -SubscriptionId $WVDConnection.SubscriptionId -ServicePrincipal
			if (!$AzAuth -or !$AzAuth.Context) {
				throw $AzAuth
			}
			$AzContext = $AzAuth.Context
			$WvdContext = $WvdAuth.Context
		}
		catch {
	...
	if ($AzContext.Tenant.Id -ne $Connection.TenantId -or $AzContext.Subscription.Id -ne $Connection.SubscriptionId) {
		if ($PSCmdlet.ShouldProcess((@($Connection.TenantId, $Connection.SubscriptionId) -join ', '), 'Set Azure context with tenant ID, subscription ID')) {
			try {
				$AzContext = Set-AzContext -TenantId $Connection.TenantId -SubscriptionId $Connection.SubscriptionId
				if (!$AzContext -or $AzContext.Tenant.Id -ne $Connection.TenantId -or $AzContext.Subscription.Id -ne $Connection.SubscriptionId) {
					throw $AzContext
				}
			}
			catch {
